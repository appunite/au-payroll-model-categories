...
def analyze_category_distribution(csv_file: str = "invoices_training_data.csv"):
    ...
    if len(rare_categories) > 0:
        ...
        print(f"   These may not train well due to insufficient data.")
        print(f"   Total records in rare categories: {rare_categories.sum()} ({rare_categories.sum()/len(df)*100:.1f}%)")
    ...
    recommendations.append({
        ...
        'impact': f'Balance specificity and training data quality',
    ...
    print(f"  Action: Remove {len(rare_categories)} categories with < {min_samples_threshold} samples")
    print(f"  Pros: Better training quality for remaining categories")
    print(f"  Cons: Lose {rare_categories.sum()} records, can't predict rare categories")

    print("\nOption 2: Use Parent Categories Only")
    print(f"  Action: Merge all subcategories to parent (e.g., people:gifts → people)")
    print(f"  Pros: More data per category, {len(parent_counts)} balanced categories")
    print(f"  Cons: Less specific predictions")

    print("\nOption 3: Hybrid Approach (RECOMMENDED)")
    print(f"  Action: Keep {len(common_subcats)} common subcategories, merge rare to parent")
    print(f"  Pros: Best balance of specificity and data quality")
    print(f"  Cons: Slightly more complex")

    print("\nOption 4: Class Weighting (Keep All)")
    print(f"  Action: Use class_weight='balanced' in LightGBM")
    print(f"  Pros: No data loss, model learns all categories")
    print(f"  Cons: May still struggle with very rare categories")
...
def apply_filter_strategy(strategy: str, min_samples: int = 30):
    ...
    print(f"  Filtered: {len(df_filtered)} records, {df_filtered['expenseCategory'].nunique()} categories")
    print(f"  Saved to: {output_path}")
    print(f"\nTo train with filtered data:")
    print(f"  python src/train_model.py invoices_training_data_filtered.csv")
